...
{
    const json j = { { "mode", "rocket" }, { "distance_km", 1.0 } };
    EXPECT_THROW(make_transit_event_from_json("alice", j, 123), std::runtime_error);
}

TEST(TransitLogic, ValidInputReturnsEvent)
{
    const json j  = { { "mode", "bike" }, { "distance_km", 2.5 } };
...
{
    const json j  = { { "mode", "walk" }, { "distance_km", 1.0 }, { "ts", 4242424242 } };
...
TEST(TransitLogic, AcceptedModesDoNotThrow)
{
    ...
    {
        const json j = { { "mode", m }, { "distance_km", 0.1 } };
...
{
    const json j = { { "mode", "walk" }, { "distance_km", 0.0 } };
