...
{
    json const j = { { "mode", "walk" }, { "distance_km", 1.0 } };
...
{
    json const j = { { "mode", "rocket" }, { "distance_km", 1.0 } };
    EXPECT_THROW(make_transit_event_from_json("alice", j, 123), std::runtime_error);
}

TEST(TransitLogic, ValidInputReturnsEvent)
{
    json const j  = { { "mode", "bike" }, { "distance_km", 2.5 } };
...
{
    json const j  = { { "mode", "walk" }, { "distance_km", 1.0 }, { "ts", 4242424242 } };
...
TEST(TransitLogic, AcceptedModesDoNotThrow)
{
    ...
    {
        json const j = { { "mode", m }, { "distance_km", 0.1 } };
